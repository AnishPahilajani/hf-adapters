...
.PHONY: help test tests adapter-coverage-tests smoke-tests load-tests \
        ...
        model-components-tests masked-lm-compare-tests question-answering-compare-tests \
...
tests: ## Run the suites selected by TEST_TYPE into RESULTS_DIR (JUnit per suite)
	...
	case " $$resolved " in \
	  *" regression "*|*" trunk "*) suites="adapter_coverage smoke load token_compare model_components embed_compare vlm reranker_compare masked_lm_compare question_answering_compare token_classification_compare model_module" ;; \
	  *" unit "*) suites="adapter_coverage load token_compare model_components embed_compare vlm reranker_compare masked_lm_compare question_answering_compare token_classification_compare model_module" ;; \
	  " integration ") suites="token_compare model_components" ;; \
	  " perf ") suites="perf" ;; \
	...
	for suite in $$suites; do \
	  ...
	  case "$$suite" in \
	    ...
	    *) echo "Unknown suite key '$$suite'. Valid: adapter_coverage smoke load token_compare model_components embed_compare vlm reranker_compare masked_lm_compare question_answering_compare token_classification_compare model_module edge_cases perf"; rc=1 ;; \
	  esac; \
	done; \
	exit $$rc
