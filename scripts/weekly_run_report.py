...
def make_report(
    ...
    title = html.escape(
        f"{run.get('name', DEFAULT_WORKFLOW)} · run {run['id']} · {run_date}"
    )
    return f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{title}</title><style>
:root{{--bg:#f7f8fa;--ink:#172033;--muted:#667085;--line:#d0d5dd;--ok:#32a852;--bad:#d64545;--live:#87ceeb;--queued:#98a2b3}}
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:16px system-ui,sans-serif}}
header{{padding:24px 28px 16px}}h1{{font-size:22px;margin:0 0 6px}}a{{color:#175cd3}}.meta{{color:var(--muted)}}
...
#queue{{margin:0 20px 14px;padding:14px 18px;background:#eef1f5;border:1px solid var(--line);border-radius:8px}}#queue h2{{font-size:17px;margin:0 0 10px}}.queue-groups{{display:flex;flex-wrap:wrap;gap:18px}}.queue-group{{min-width:180px}}.queue-group h3{{font-size:14px;margin:0 0 5px}}.queue-jobs{{display:flex;flex-wrap:wrap;gap:6px}}.queue-job{{display:inline-block;padding:4px 8px;border-radius:5px;background:var(--queued);color:#101828;text-decoration:none;font-size:14px;font-weight:650}}.queue-job:hover{{outline:2px solid var(--ink)}}
#wrap{{overflow:auto;margin:0 20px 28px;background:white;border:1px solid var(--line);border-radius:8px}}svg{{display:block;min-width:1100px}}.axis,.grid{{stroke:var(--line)}}.grid{{stroke-dasharray:3 4}}.label{{fill:var(--ink);font-size:14px;font-weight:500}}.tick{{fill:var(--muted);font-size:14px}}.bar{{stroke:#fff;stroke-width:1;rx:3;cursor:pointer}}.bar:hover{{stroke:var(--ink);stroke-width:2}}.kind{{fill:#101828;font-size:13px;font-weight:800;text-anchor:middle;pointer-events:none}}.empty{{fill:#f2f4f7;stroke:#e4e7ec}}
#tooltip{{display:none;position:fixed;z-index:10;max-width:520px;padding:10px 12px;border-radius:6px;background:#101828;color:white;font-size:15px;line-height:1.5;white-space:pre-line;pointer-events:none;box-shadow:0 4px 14px #0003}}
</style></head><body><header><h1>{title}</h1><div class="meta"><a href="{html.escape(run.get('html_url', ''))}">Open GitHub run</a> · event: {html.escape(run.get('event', 'unknown'))} · status: {html.escape(run.get('status', 'unknown'))}</div>
...
DATA.rows.forEach((row,i)=>{{const y=chartTop+i*rowH;if(row.section_start){{add('line',{{x1:0,y1:y,x2:width,y2:y,stroke:'#667085','stroke-width':2}});const heading=add('text',{{x:18,y:y+21,class:'label','font-weight':'800'}});heading.textContent=row.group;}}let q=add('text',{{x:left-10,y:y+21,'text-anchor':'end',class:'label'}});q.textContent=row.label;add('line',{{x1:left,y1:y+rowH,x2:width-right,y2:y+rowH,class:'axis'}});if(!row.jobs.length)add('rect',{{x:left,y:y+7,width:3,height:18,class:'empty'}});row.jobs.forEach(j=>{{const s=j.started_at?Date.parse(j.started_at):DATA.end,e=j.completed_at?Date.parse(j.completed_at):DATA.end,w=Math.max(24,x(e)-x(s)),cls=!j.completed_at?'var(--live)':j.conclusion==='success'?'var(--ok)':'var(--bad)',kind=j.name.includes('(embedding ')?'E':j.name.includes('(generative ')?'G':'',kindName=kind==='E'?'Embedding':kind==='G'?'Generative':'Other',link=add('a',{{href:j.html_url,target:'_blank',rel:'noopener'}}),r=add('rect',{{x:x(s),y:y+5,width:w,height:22,fill:cls,class:'bar'}},link),kindLabel=add('text',{{x:x(s)+w/2,y:y+21,class:'kind'}},link),start=j.started_at?formatTime(s):'Not started',end=j.completed_at?formatTime(e):'Still running',details=`${{j.name}}\nType: ${{kindName}}\nStatus: ${{j.conclusion||j.status}}\nStart: ${{start}}\nEnd: ${{end}}\nDuration: ${{formatDuration(e-s)}}`;kindLabel.textContent=kind;r.addEventListener('mouseenter',()=>{{tooltip.textContent=details;tooltip.style.display='block';}});r.addEventListener('mousemove',event=>{{tooltip.style.left=`${{Math.min(event.clientX+14,innerWidth-tooltip.offsetWidth-12)}}px`;tooltip.style.top=`${{Math.min(event.clientY+14,innerHeight-tooltip.offsetHeight-12)}}px`;}});r.addEventListener('mouseleave',()=>tooltip.style.display='none');r.addEventListener('contextmenu',async event=>{{event.preventDefault();await copyText(j.html_url);tooltip.textContent='Link copied to clipboard';tooltip.style.display='block';tooltip.style.left=`${{Math.min(event.clientX+14,innerWidth-tooltip.offsetWidth-12)}}px`;tooltip.style.top=`${{Math.min(event.clientY+14,innerHeight-tooltip.offsetHeight-12)}}px`;setTimeout(()=>tooltip.style.display='none',1200);}});}})}});
</script></body></html>"""
