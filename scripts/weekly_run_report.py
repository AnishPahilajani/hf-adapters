...
#wrap{{overflow:auto;margin:0 20px 28px;background:var(--surface);border:1px solid var(--line);border-radius:8px}}svg{{display:block;min-width:1100px}}.axis,.grid{{stroke:var(--line)}}.grid{{stroke-dasharray:3 4}}.label{{fill:var(--ink);font-size:14px;font-weight:500}}.tick{{fill:var(--muted);font-size:14px}}.bar{{stroke:var(--surface);stroke-width:1;rx:3;cursor:pointer}}.bar:hover{{stroke:var(--ink);stroke-width:2}}.kind{{fill:var(--bar-text);font-size:13px;font-weight:800;text-anchor:middle;pointer-events:none}}.empty{{fill:var(--panel);stroke:var(--line)}}
...
</style></head><body><header><div class="title-row"><h1>{title}</h1><button id="theme-toggle" type="button">Light theme</button></div><div class="meta"><a href="{html.escape(run.get('html_url', ''))}">Open GitHub run</a> · event: {html.escape(run.get('event', 'unknown'))} · status: {html.escape(run.get('status', 'unknown'))}</div>
